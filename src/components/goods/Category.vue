<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 商品分类列表区域 -->
      <tree-table class="treeTable" :data="categorylist" :columns="columns" :selection-type="false" :expand-type="false" show-index index-text="#" border :show-row-hover="false">
        <!-- 是否有效 -->
...
export default {
  ...
  methods: {
    ...
    async getCategoryList() {
      ...
      // 把数据列表赋值给 categorylist
      this.categorylist = res.data.result
      // 为总数据条数赋值
      this.total = res.data.total
    },
    // 监听 pagesize 改变的事件，即每页多少条数据的改变
    handleSizeChange(newSize) {
      this.queryInfo.pagesize = newSize
      this.getCategoryList()
    },
    // 监听 pagenum 改变的事件，即当前的页码值发生改变
